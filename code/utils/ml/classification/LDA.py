...
def LDA(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame) -> tuple[dict[str, float], object, list[str]]:
    """
    Train a Linear Discriminant Analysis (LDA) classifier with hyperparameter tuning 
    and evaluate its performance on the test set.

    LDA is a classification method that finds a linear combination of features that best separates two or more classes.
    ...
    :return: A tuple containing:
        - model_metrics (dict): Performance metrics:
            ...
            - "Weighted Recall"
            - "Weighted F1-Score"
        - best_lda (object): Trained LinearDiscriminantAnalysis model with best hyperparameters.
    ...
    param_grid = [
        ...
        {'solver': ['lsqr', 'eigen'], 'shrinkage': ['auto', None]}  # LSQR and Eigen support shrinkage
    ]
    ...
    return model_metrics, best_lda, features