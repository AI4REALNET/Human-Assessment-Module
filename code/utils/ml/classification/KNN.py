...
def KNN(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame) -> tuple[dict[str, float], object, list[str]]:
    """
    Train a K-Nearest Neighbors (KNN) classifier with hyperparameter tuning 
    and evaluate its performance on the test set.

    KNN is a non-parametric classification method that assigns labels based 
    on the majority class of the nearest neighbors in the feature space.
    ...
    :return: A tuple containing:
        - model_metrics (dict): Performance metrics:
            ...
            - "Weighted Recall"
            - "Weighted F1-Score"
        - best_knn (object): Trained KNeighborsClassifier model with best hyperparameters.
